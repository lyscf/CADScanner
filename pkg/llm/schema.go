...
// UnmarshalJSON accepts both the current "reason" field and the legacy
// "reasoning" field to preserve cache compatibility.
func (s *SemanticAnalysis) UnmarshalJSON(data []byte) error {
	type semanticAnalysisAlias struct {
		Provider         string        `json:"provider,omitempty"`
		Model            string        `json:"model,omitempty"`
		Sample           string        `json:"sample,omitempty"`
		InputType        string        `json:"input_type,omitempty"`
	...
	var aux semanticAnalysisAlias
	if err := json.Unmarshal(data, &aux); err != nil {
		return err
	}

	s.Provider = aux.Provider
	s.Model = aux.Model
	s.Sample = aux.Sample
	s.InputType = aux.InputType
	s.CacheKey = aux.CacheKey
	s.CacheKeyVersion = aux.CacheKeyVersion
	s.EncodingHash = aux.EncodingHash
	s.CacheHit = aux.CacheHit
	s.SemanticLabel = aux.SemanticLabel
	s.Confidence = aux.Confidence
	s.ThreatPatterns = aux.ThreatPatterns
	s.IOCs = aux.IOCs
	s.AttackMapping = aux.AttackMapping
	s.Reason = aux.Reason
	if s.Reason == "" {
		s.Reason = aux.LegacyReasoning
	}
	s.TriageReport = aux.TriageReport
	s.PromptTokens = aux.PromptTokens
	s.CompletionTokens = aux.CompletionTokens
	s.LatencyMs = aux.LatencyMs
	s.RawResponse = aux.RawResponse
	s.Error = aux.Error
	return nil
}
